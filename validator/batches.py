...
def get_depths_params(param_filters):
    ...
    param_name_list = [pfil.filter.name for pfil in param_filters]
...
def create_jobs(
    ...
    if (
        isinstance(reader, GriddedBase)
        or isinstance(reader, ReaderWithTsExtension)
        or isinstance(reader, ReaderWithExtension_SMAP)
    ):
