...
def get_inspection_table(validation_run:ValidationRun):
    ...
    if bool(outfile) and path.exists(outfile):
        ...
        # Check that file size is less than 100 MB
            ...
            warnings.warn(f"File size of {file_size} bytes is too large to be read")
...
def encoded_comparisonPlots(
    ...
    comparison.wrapper(method=plot_type, metric=metric)
