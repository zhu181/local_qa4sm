...
def _resolve_config_ref(
	raw_ref: Any, config_by_id: dict[int, DatasetConfiguration]
) -> Optional[DatasetConfiguration]:
	if raw_ref is None:
		return None
...
def parse_validation_run_config(payload: dict[str, Any]) -> ValidationRun:
	...
	dataset_configs = Collection(
		_parse_dataset_configuration(cfg, val_run) for cfg in raw_dataset_configs
	)
	config_by_id = {cfg.id: cfg for cfg in dataset_configs}

	spatial_ref = _resolve_config_ref(
		raw.get("spatial_reference_configuration"), config_by_id
	)
	temporal_ref = _resolve_config_ref(
		raw.get("temporal_reference_configuration"), config_by_id
	)
